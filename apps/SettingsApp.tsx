          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {WALLPAPERS.map((wp, i) => (
                  <button key={i} onClick={() => save('user.desktop.wallpaper', wp)} className="aspect-video rounded-lg overflow-hidden border-2 border-transparent hover:border-blue-500"><img src={wp} className="w-full h-full object-cover"/></button>
                ))}
              </div>
            </div>
            <div>
              <label className="text-[10px] font-black uppercase text-gray-500 mb-4 block">Brightness (${br}%)</label>
              <input type="range" min="20" max="100" value={br} onChange={(e) => { setBr(Number(e.target.value)); save('user.display.brightness', e.target.value); }} className="w-full"/>
            </div>
          </div>
        )}
      </div>
