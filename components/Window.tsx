...
export const Window: React.FC<any> = ({ state, onClose, onMinimize, onMaximize, onFocus, onUpdate, children }) => {
  return (
    <div 
      ...
      style={{ 
        left: state.isMaximized ? 0 : state.x, 
        top: state.isMaximized ? 0 : state.y, 
        width: state.isMaximized ? '100vw' : state.width, 
        height: state.isMaximized ? '100vh' : state.height, 
        zIndex: state.zIndex, 
    ...
    >
      <div className="h-10 bg-white/5 border-b border-white/5 flex items-center px-4 shrink-0 cursor-move">
        ...
        <div className="flex gap-2 pointer-events-auto">
          ...
          <button onClick={(e) => { e.stopPropagation(); onClose(state.id); }} className="p-1 hover:bg-red-500/20 text-red-500 rounded z-50"><X size={14}/></button>
        </div>
      </div>
