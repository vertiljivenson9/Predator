...
export class VFS implements IVFS {
  ...
  mount(path: string, backend: FileSystemBackend) { this.mounts.push({path, backend}); this.mounts.sort((a,b) => b.path.length - a.path.length); }
  private resolve(path: string) { const clean = path === '/' ? '/' : path.replace(/\/$/, ''); for (const m of this.mounts) { const p = m.path === '/' ? '/' : m.path + '/'; if (clean === m.path || clean.startsWith(p)) return { backend: m.backend, rel: clean }; } return null; }
  async ls(path: string) { const r = this.resolve(path); return r ? r.backend.ls(r.rel) : []; }
