...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const handleSync = () => syncHardware();
    window.addEventListener('sys-config-update', handleSync);
    return () => window.removeEventListener('sys-config-update', handleSync);
  }, [syncHardware]);
  ...
  const renderContent = (win: WindowState) => {
    switch(win.appId) {
      ...
      default: return <div className="p-4 text-white font-mono">Module Load Success: ${win.appId}</div>;
    }
  };
  ...
  return (
    <div 
        className="h-screen w-screen overflow-hidden relative font-sans text-white select-none touch-none bg-black"
